...
if [ $USER != 'root' ]; then
	echo "Anda harus menjalankan ini sebagai root"	exit
fi
...
wget -O /etc/nginx/nginx.conf "https://raw.githubusercontent.com/masjeho/ocs_panel/master/nginx.conf"
wget -O /etc/nginx/conf.d/vps.conf "https://raw.githubusercontent.com/masjeho/ocs_panel/master/vps.conf"
...
wget https://raw.github.com/masjeho/ocs_panel/master/Ocs.zip
unzip Ocs.zip
rm -f Ocs.zip
...
chmod 644 /home/vps/public_html/config/config.ini
chmod 644 /home/vps/public_html/config/route.ini
...
echo "" | tee -a log-install.txt
echo "Auto Script Installer OCS Panels | radenmasjeho.blogspot.com"  | tee -a log-install.txt
echo "             (http://radenmasjeho.blogspot.com/ - 081366652270)           "  | tee -a log-install.txt
echo "" | tee -a log-install.txt
